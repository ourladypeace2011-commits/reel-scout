...
#### 🔴 6C spike 結果（2026-09-02）：訊號成立，但**分類器不能出貨**，原因不在演算法

實作到一半停下來的紀錄，寫在這裡是為了讓下一次不要重跑同一輪。

**① 位移訊號本身是實的、而且便宜。** 全片一次 ffmpeg pass 解成 64×64 灰階 @8fps
（4 分鐘的片＝1,942 楨、約 8 MB），純 numpy FFT 相位相關求整數像素位移。
`Wax On Wax Off` 實測：**61% 的相鄰幀零位移**（大量鎖定機位），p90 = 2.24px。

**② 統計量要用中位數不能用平均。** 有顆鏡頭中位數 0.00 但平均 4.16 ——
少數幾楨的大跳動把平均整個帶走。

**③ 🔴 但分類的正確性取決於鏡頭邊界，而邊界有漏。** 一顆被判為
「方向一致度 0.98、持續同向移動」的鏡頭（#15，73.78–78.95s），**抽出首尾兩幀肉眼比對，
是兩組完全不同的素材**（一邊是人物特寫、一邊是五人道服全景）——
那個 span 裡有一個 scene 偵測（門檻 0.3）漏掉的剪點。
**漏掉的剪點在數值上長得跟一個漂亮的搖鏡一模一樣。** 差一點就把它當量測出貨。

**④ 用相位相關的峰值高度當可信度來擋剪點 —— 實測推翻。** 直覺是「兩幀不相干時峰值低」，
實際相反：**跨剪點 p50 = 0.53，鏡頭內 p50 = 0.36**。因為這支片大量黑底，
兩張不相干的暗幀在零位移處相關性很強；反而有真實紋理與運動的鏡頭內幀對峰值較低較散。
任何門檻都是「擋不到剪點、卻誤擋鏡頭內」。

**⑤ 結論：6C 在 cut 偵測補強之前不要出分類器。** 現在出等於把「漏剪點」
包裝成「運鏡判讀」，而那正是 §4E 整條在解的那種病 —— 一個看起來有信心、
實際沒有依據的值。可走的方向（未驗）：先用位移訊號**反過來補剪點偵測**
（一顆鏡頭內部出現與周圍不連續的大位移＝疑似漏剪），把邊界修好之後再談運鏡。
